...
description: SPD V1 Batch. Process large, user-supplied sets of legitimate directory URLs across Windows, macOS, and Linux-capable environments through normalization, deduplication, execution sharding, verification-first queues, authorization-controlled browser/API work, idempotent submission, recovery, and truthful throughput reporting. Use when coverage and operational throughput matter more than deep per-site quality analysis. Do not use for ranking manipulation, bulk link spam, invented data, CAPTCHA bypass, paid-link acquisition, forced reciprocal links, or routes prohibited by a site's terms.
---
...
1. Read the verified project/product profile, brand rules, contact and credential aliases, approved assets, source list, batch authorization, and existing record.
...
Do **not** create or run ad-hoc per-site HTML parsers, guessed `requests`/`urllib` POSTs, hidden-field scrapers, or GitHub Actions that imitate browser form submissions. A browser route can depend on JavaScript, cookies, CSRF/session state, bot protection, or interactive terms that a generic HTTP client does not reproduce. If no safe browser/API backend is available, preserve the queue item as a handoff instead of pretending it was automated.

## Apply the batch legitimacy gate

Reject or separate any route that is irrelevant to the project, unavailable, unreleased-only, paid-link-only, forced-reciprocal, a known low-quality directory network, or prohibited for automated form work. Do not select sites because they promise dofollow links, ranking gains, DA/DR, or backlink volume.

Use only the exact brand, project/product name, or naked canonical URL as public link text. Never request dofollow treatment or use repeated commercial exact-match anchors.
...
2. Derive an idempotency key from platform domain, canonical project/product ID, account alias, and route.
...
1. Run a read-only preflight over each shard before entering listing fields.
...
5. Submit sequentially within a browser profile unless an official API is the selected route. Record the result before advancing the queue cursor.
